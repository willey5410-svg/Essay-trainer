...
- 起動時にクラウドから取得するため、**別の端末でも合言葉を入れるだけで同じデータが使えます**（Blob の反映はCDNキャッシュの都合で最大1分程度遅れることがあります）
- Blob のパスは合言葉から導出したハッシュを含むため、合言葉を知らない第三者は URL を推測できません（合言葉を変更すると新しいデータ領域になります）
